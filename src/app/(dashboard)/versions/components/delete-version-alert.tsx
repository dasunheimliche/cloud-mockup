...
export default function DeleteVersionAlert({
  selectedVersion,
  loading,
  onConfirm,
}: any) {
  ...
  return (
    <Modal className="bg-white overflow-hidden">
      <div className="flex gap-2 items-center text-sm font-bold px-5  text-[#ffffffcb] bg-gradient-to-l from-[#01110b]  to-[#010618] w-full h-14">
        <PiStackFill size={"1.25rem"} /> {"DELETE INSTANCE"}
      </div>
      <div className="p-4">
        {"The version of "} <b>{"type"}</b>{" "}
        <b className="text-red-600">{selectedVersion.type}</b> {" and "}{" "}
        <b>{"version "}</b>
        <b className="text-red-600">{selectedVersion.version}</b>
